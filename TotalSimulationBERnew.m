...
% wait_bar = waitbar(0,'Constructing additional pylons...');
...
%      sum(stream_channel~=stream_rec_CFOPhase)        % if equal to 0 no CFO was added
    ...
    %% Apply window
    stream_rec_CFOPhase = stream_wind;              %%%%!!!!!!%%%%!!!
...
%     [epsilon2(ii,:), ~] = GardnerNoel(stream_rec_sample, K_gard, stream_rec_wind, ftaps, fs, T, t0);
    %stream_rec_gardner = stream_rec_sample(1:2:end);
   end

    error = (t0/T-abs(epsilon))*T;
%     error2 = abs(abs(epsilon2)-t0/T);

    avg = mean(error,1);
    stddev = std(error,1);
    steps = 1:16:numel(avg);
    figure
    plot(avg)
    title(['K = ',num2str(K_gard)])
    hold on
    plot(avg-stddev)
    plot(avg+stddev)
    hold off
